...
(define number-reverse
  (compose string->number
           string-reverse
           number->string))

(define (palindrome? x)
  (eqv? x (number-reverse x)))

(define (lychrel? x accuracy)
  (or (negative? accuracy)
    (let ((result (+ x (number-reverse x))))
      (and (not (palindrome? result))
        (lychrel? result (1- accuracy))))))

(display
  (count (cut lychrel? <> 50)
         (iota 10000 1)))
(newline)
