...
function zip(xs) {
  return Array.from(xs, (_, i) => xs.map(x => x[i]))
}
...
function diagonal(x) {
  ...
  tl = zip(tl)
  tr = zip(tr)
  return tl.concat(tr).map(compact);
}

function findMaxProduct(xs) {
  const res = [];
  for (let i = 0, len = xs.length; i < len; i++) {
    res.push(product(xs.slice(i, i + 4)));
  }
  return Math.max(...res);
}

const answer = Math.max(...[horizonal, vertical, diagonal].map((f) =>
  Math.max(...f(matrix).map(findMaxProduct))
))
console.log(answer)
